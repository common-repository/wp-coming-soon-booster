<?php
/**
 * Template for contact form data settings.
 *
 * @author  Tech Banker
 * @package wp-coming-soon-booster/views/contact-form-data
 * @version 1.0.0
 */
...
if ( ! is_user_logged_in() ) {
	...
	if ( ! $access_granted ) {
		return;
	} elseif ( CONTACT_FORM_DATA_COMING_SOON_BOOSTER === '1' ) {
		...
		<div class="row">
			<div class="col-md-12">
				...
				<div class="portlet-body form">
					<form id="ux_frm_contact_form_data">
						<div class="form-body">
							<div class="row">
								<div class="col-md-6">
							...
							<div class="col-md-6">
								<div class="form-group">
									...
									<input type="text" class="form-control" onkeypress="prevent_datepicker('#ux_txt_end_date');" name="ux_txt_end_date" id="ux_txt_end_date" value="<?php echo esc_attr( date( 'm/d/Y' ) ); ?>" placeholder="<?php echo esc_attr( $csb_end_date_placeholder ); ?>">
						...
						<div class="form-actions">
							<div class="pull-right">
								<input type="submit" class="btn vivid-green" id="btn_save_start_end_date_contact" name="btn_save_start_end_date_contact" value="<?php echo esc_attr( $csb_submit ); ?>">
							</div>
						</div>
						<div class="line-separator"></div>
							<div class="table-margin-top">
								<select name="ux_ddl_apply" id="ux_ddl_apply" class="custom-bulk-width">
									...
									<option value="delete" style="color:red";><?php echo esc_attr( $csb_delete ); ?><span> ( <?php echo esc_attr( $csb_premium_edition_label ); ?> )</span></option>
								</select>
								<input type="button" class="btn vivid-green" name="ux_btn_contact_formdata_Apply" id="ux_btn_contact_formdata_Apply" value="<?php echo esc_attr( $csb_apply ); ?>" onclick = "premium_edition_notification_coming_soon_booster();">
							</div>
							<div class="contact-form-scroll">
								<table class="table table-striped table-bordered table-hover table-margin-top" id="ux_tbl_contact_form_data">
									<thead>
									<tr>
										<th style="text-align:center;" class="chk-action">
											<input type="checkbox" name="ux_chk_contact_form_data" id="ux_chk_contact_form_data">
										</th>
										<th style="width:15%;">
											<?php echo esc_attr( $csb_name ); ?>
										</th>
										<th style="width:15%;">
											<?php echo esc_attr( $csb_contact_form_data_email ); ?>
										</th>
										<th style="width:20%;">
									...
									<tbody id="dynamic_table_filter">
										<?php
										if ( isset( $contact_form_details_array ) && count( $contact_form_details_array ) > 0 ) {
											foreach ( $contact_form_details_array as $keys ) {
												?>
												<tr>
													<td style="text-align:center;">
														<input type="checkbox" id="ux_chk_coming_soon_<?php echo esc_attr( $keys['meta_id'] ); ?>" name="ux_chk_coming_soon_<?php echo esc_attr( $keys['meta_id'] ); ?>" value="<?php echo esc_attr( $keys['meta_id'] ); ?>" onclick="checkAllData_contact_form_data_coming_soon_booster(<?php echo esc_attr( $keys['meta_id'] ); ?>);">
													</td>
													<td style="width:15%;">
														<label class="control-label">
															<?php echo esc_attr( $keys['name'] ); ?>
														</label>
													</td>
													<td style="width:15%;">
														<label class="control-label">
													...
													<td style="width:20%;">
														<label class="control-label">
															<?php echo esc_attr( $keys['message'] ); ?>
														</label>
													</td>
													<td style="width:20%;">
														<label class="control-label">
													...
													<td class="custom-alternative">
														<a href="javascript:void(0);">
															<a class="btn coming-soon-buttons" onclick="delete_data_coming_soon_booster('<?php echo esc_attr( $keys['meta_id'] ); ?>', '<?php echo esc_attr( $csb_form_entries_delete ); ?>', 'admin.php?page=csb_contact_form_data');"><?php echo esc_attr( $csb_delete ); ?> </a>
														</a>
													</td>
												</tr>
												<?php
